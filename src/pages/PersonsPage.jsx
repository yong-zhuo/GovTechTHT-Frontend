...
import {
  getAverageCumulativeGpas,
  getAverageSemesterGpas,
} from '../utils/gpaUtils'
...
const PersonsPage = ({ data, tableColumns, students }) => {
  ...
  return (
    <div className="flex flex-col p-5 sm:ml-5">
      <span className="text-3xl font-semibold text-slate-400 text-center sm:text-left capitalize">
        {role}
      </span>
      <div className="flex flex-col items-center justify-center">
        <div className="flex flex-col bg-white rounded-lg shadow p-5 mt-5 w-full sm:w-3/4">
          <GPADoubleLineChart
            firstDataSet={averageSemesterGpas}
            secondDataSet={averageCumulativeGpas}
          />
        </div>
        <div className="flex flex-col bg-white rounded-lg shadow p-5 mt-5 w-full sm:w-3/4">
          <span className="text-xl mb-3 font-semibold capitalize">
            List of {role}
          </span>
